To make the `graph_container` invisible while still maintaining the colorful and bold outer border effect, you can set its `stroke` and `stroke-width` to `transparent` and `0px`, respectively. Here's how you can modify the code:

```mermaid
graph TD;

  subgraph graph_container
    style graph_container fill:transparent, stroke:transparent, stroke-width:0px, stroke-dasharray: 0;
    ...
    style graph_container stroke:transparent, stroke-width:0px; /* Make outer border invisible */
